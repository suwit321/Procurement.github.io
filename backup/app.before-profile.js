  ...
  function renderProjectModal(projectId) {
    ...
    U.setHTML('detailModalBody', `
      <div class="detail-section">
        ...
        <div class="detail-cart-row">${rows.length === 1 ? cartBtn(r, { label: true })
          : `<button type="button" class="cart-add-all" data-cart-add-project="${U.esc(r.project_id)}">+ เพิ่มทั้ง ${rows.length} สัญญาลงตะกร้า</button>`}</div>
      ...
      ${rows.length > 1 ? `<div class="detail-section">
        ...
        <table class="table table-sm mini-table mb-0">
          ...
          <tbody>${rows.map(x => `<tr><td>${cartBtn(x)}${U.esc(x.contract_no)}</td>

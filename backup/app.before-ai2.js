...
const App = (() => {
  ...
  function popupHTML(r, group) {
    ...
    return `
      <div class="map-popup">
        ...
        <div class="detail-cart-row mt-2">${cartBtn(r, { label: true })}</div>
        <button class="btn btn-sm btn-outline-primary w-100 detail-clickable"
  ...
  /* =========================================================
     ...
     ระบบจึงสรุปเฉพาะส่วนที่เกี่ยวกับงานนั้น ๆ เป็นข้อความสั้น แล้วแนบไปกับคำถาม
     ผู้ใช้เปิดดูได้ทุกครั้งว่าส่งอะไรออกไป
  ...
  const ai = { cfg: null, thread: [], busy: false, controller: null, lastRecord: null, wired: false };
...
3. สัญญาณเสี่ยงเป็นเหตุผลให้ตรวจสอบต่อ ไม่ใช่หลักฐานการทุจริต ห้ามกล่าวหาบุคคลหรือนิติบุคคลใด ให้ระบุคำอธิบายทางเลือกที่สุจริตเมื่อเกี่ยวข้อง
4. เมื่ออ้างถึงสัญญาใด ให้ใส่รหัสโครงการในรูป [P:รหัสโครงการ] ตรงตามที่ปรากฏในข้อมูล
5. คะแนนความเสี่ยงคือผลรวมน้ำหนักของกฎ ไม่ใช่ความน่าจะเป็น
6. ตอบเป็นภาษาไทย กระชับ ใช้หัวข้อ รายการ และตารางแบบ Markdown เมื่อช่วยให้อ่านง่าย จำนวนเงินใช้หน่วยบาทหรือล้านบาท
7. ถ้าข้อจำกัดของข้อมูลกระทบข้อสรุป ให้บอกไว้ท้ายคำตอบสั้น ๆ`;
  ...
  function aiRecordLine(r, { rules = true } = {}) {
    ...
    const parts = [
      ...
      rules && (r.rule_hits || []).length ? `กฎ: ${r.rule_hits.map(h => h.rule_id).join(',')}` : null,
    ];
  ...
  /* ผู้ให้บริการที่บริบทสั้นหรือโควตาโทเค็นต่อนาทีต่ำ (แพ็กเกจฟรีของ Groq) รับข้อมูลเต็มไม่ไหว
     จึงส่งฉบับย่อ: รายการสัญญาและหน่วยงานน้อยลง ตัดคำอธิบายกลุ่มงาน */
  const compactMode = () => !!AI.PROVIDERS[ai.cfg.provider]?.compact;

  function aiContextFilter() {
    const rows = state.filtered;
    const small = compactMode();
    const total = U.sum(rows.map(r => r.contract_price_agree));
    ...
    const agencies = Analytics.agencyTotals(rows)
      .sort((a, b) => b.n_flagged - a.n_flagged || b.total_value - a.total_value).slice(0, small ? 5 : 10)
    ...
    const contractors = Analytics.contractorTotals(rows).slice(0, small ? 5 : 10)
    ...
    const cliff = Analytics.thresholdCliff(rows);
    const top = rows.filter(r => r.risk_score > 0).slice(0, small ? 6 : 15).map(r => `- ${aiRecordLine(r)}`).join('\n');
    return [
      `ขอบเขต: ${U.$('gfSummary').textContent.replace(/✕|ล้างทั้งหมด/g, '').replace(/\s+/g, ' ').trim()}`,
      `จำนวน ${U.num(rows.length)} สัญญา · มูลค่ารวม ${baht(total)} · วันลงนาม ${dates[0] || '-'} ถึง ${dates[dates.length - 1] || '-'}`,
      `\nระดับความเสี่ยง:\n${bands}`,
      `\nกฎที่พบบ่อย:\n${ruleCountLines(rows, small ? 6 : 12)}`,
  ...
  function aiContextEntity(kind, key) {
    ...
    const total = U.sum(rows.map(r => r.contract_price_agree));
    const dates = rows.map(r => r.contract_date).filter(Boolean).sort();
    const other = (isC ? Analytics.agencyTotals(rows) : Analytics.contractorTotals(rows)).slice(0, small ? 5 : 12)
    ...
    const discs = rows.filter(r => r.price_build > 0 && r.contract_price_agree !== null && (projectCounts().get(r.project_id) || 1) === 1)
      .map(r => 1 - r.contract_price_agree / r.price_build).sort((a, b) => a - b);
    const top = [...rows].sort((a, b) => b.risk_score - a.risk_score || b.contract_price_agree - a.contract_price_agree)
      .slice(0, small ? 5 : 12).map(r => `- ${aiRecordLine(r)}`).join('\n');
    return [
      ...
      conc ? `ความเข้มข้นของผู้รับจ้าง: HHI ${conc.hhi} · CR4 ${U.pct(conc.cr4)} · ผู้รับจ้าง ${U.num(conc.n_contractors)} ราย` : '',
      discs.length ? `ส่วนลดเทียบราคากลาง (เฉพาะโครงการสัญญาเดียว ${U.num(discs.length)} สัญญา): ค่ากลาง ${(U.quantile(discs, 0.5) * 100).toFixed(1)}% · ไม่ลดเลย ${U.num(discs.filter(d => d <= 0).length)} สัญญา` : '',
      `\nวิธีจัดหา:\n${countLines(U.countBy(rows, r => r.purchase_method_name), rows.length, 6)}`,
      `\n${isC ? 'หน่วยงานที่จ้างมากที่สุด' : 'ผู้รับจ้างที่ได้งานมากที่สุด'}:\n${other}`,
      `\nกฎที่พบบ่อย:\n${ruleCountLines(rows, small ? 5 : 10)}`,
      `\nสัญญาคะแนนสูงสุด:\n${top}`,
      `\nความหมายของรหัสกฎ:\n${ruleLegend(rows)}`,
      `\nข้อจำกัดของข้อมูล:\n${AI_CAVEATS.map(c => `- ${c}`).join('\n')}`,
    ].filter(Boolean).join('\n');
  ...
  const AI_TASKS = [
    { id: 'brief', icon: '📋', label: 'สรุปสำหรับผู้บริหาร', desc: 'สรุปภาพรวมตามตัวกรองปัจจุบัน ประเด็นสำคัญ และสัญญาที่ควรตรวจก่อน',
      build: () => ({
        data: aiContextFilter(),
        prompt: 'เขียนสรุปสำหรับผู้บริหารไม่เกิน 1 หน้า ประกอบด้วย (1) ภาพรวมใน 3 บรรทัด (2) ประเด็นความเสี่ยงสำคัญ 3-5 ข้อ พร้อมตัวเลขอ้างอิง (3) หน่วยงานหรือผู้รับจ้างที่ควรจับตาและเหตุผล (4) สัญญาที่ควรตรวจก่อน 5 รายการพร้อมเหตุผลสั้น ๆ (5) ข้อควรระวังในการตีความ',
      }) },
    { id: 'falsepos', icon: '🧪', label: 'ตรวจทานผลบวกลวง', desc: 'ให้ AI ชี้ว่าสัญญาคะแนนสูงรายการใดน่าจะติดธงเพราะลักษณะของข้อมูล ไม่ใช่ความเสี่ยงจริง',
      build: () => ({
        data: aiContextFilter(),
        prompt: 'ในฐานะนักวิทยาศาสตร์ข้อมูล ตรวจทานสัญญาคะแนนสูงสุดในข้อมูลว่ารายการใดน่าจะเป็นผลบวกลวงจากลักษณะของข้อมูล (เช่น ราคากลางทั้งโครงการเทียบกับสัญญาย่อย พิกัดสำนักงาน ชื่อสะกดต่างกัน) และรายการใดยังควรตรวจจริง ตอบเป็นตาราง: สัญญา | ข้อสังเกต | น่าจะเป็นผลบวกลวงหรือไม่ | เหตุผล แล้วเสนอการปรับกฎหรือเกณฑ์ที่ช่วยลดผลบวกลวง',
      }) },
    { id: 'contract', icon: '🔎', label: 'อธิบายสัญญาที่เลือก', desc: 'อธิบายว่าทำไมสัญญานี้เสี่ยง มีคำอธิบายอื่นไหม และควรขอเอกสารอะไร', needs: 'record',
      build: r => ({
        data: aiContextContract(r),
        title: truncate(r.project_name, 70),
        prompt: 'อธิบายสัญญานี้ให้ผู้ตรวจที่ไม่ใช่นักสถิติเข้าใจ: (1) สรุป 2 บรรทัดว่าทำไมระบบจัดระดับนี้ (2) สัญญาณแต่ละข้อหมายความว่าอะไร และมีคำอธิบายทางเลือกที่สุจริตอะไรบ้าง (3) การเทียบกับงานแบบเดียวกันบอกอะไร (4) เอกสารที่ควรขอและคำถามที่ควรถามหน่วยงาน เรียงตามความสำคัญ (5) ความมั่นใจโดยรวมว่าควรตรวจต่อ (สูง/กลาง/ต่ำ) พร้อมเหตุผล',
      }) },
    { id: 'letter', icon: '✉️', label: 'ร่างหนังสือขอเอกสาร', desc: 'ร่างหนังสือขอเอกสารและคำชี้แจงจากหน่วยงานสำหรับสัญญาที่เลือก', needs: 'record',
      build: r => ({
        data: aiContextContract(r),
        title: truncate(r.project_name, 70),
    ...
    { id: 'cart', icon: '🛒', label: 'ร่างบันทึกจากตะกร้า', desc: 'รวมสัญญาในตะกร้าเป็นร่างบันทึกข้อตรวจพบเบื้องต้น ใช้หมายเหตุของคุณประกอบ', needs: 'cart',
      build: () => ({
        data: aiContextCart(),
        prompt: 'ร่างบันทึกข้อตรวจพบเบื้องต้นเพื่อหารือภายใน จากสัญญาในตะกร้าและหมายเหตุของผู้ตรวจ ประกอบด้วย (1) วัตถุประสงค์และขอบเขต (2) ตารางสรุป: สัญญา | หน่วยงาน | มูลค่า | ประเด็นหลัก (3) ประเด็นร่วมที่พบข้ามหลายสัญญา (4) ข้อเสนอแนะการตรวจขั้นต่อไป เรียงตามลำดับความสำคัญ (5) ข้อจำกัดของข้อมูล',
      }) },
    { id: 'contractor', icon: '🏗️', label: 'วิเคราะห์ผู้รับจ้าง', desc: 'รูปแบบการได้งาน การพึ่งพาหน่วยงาน และสัญญาณที่เกิดซ้ำ', needs: 'contractor',
      build: key => {
        const data = aiContextEntity('contractor', key);
        return data && {
          data, title: truncate(key, 60),
          prompt: 'วิเคราะห์รูปแบบการได้งานของผู้รับจ้างรายนี้: ขนาดและการกระจายของงาน การพึ่งพาหน่วยงานใดหน่วยงานหนึ่ง วิธีจัดหาที่ได้งาน สัญญาณเสี่ยงที่เกิดซ้ำ สัญญาที่ควรดูก่อน 3-5 รายการ และคำอธิบายทางเลือกที่ไม่ใช่ความผิดปกติ',
        };
      } },
    { id: 'agency', icon: '🏛️', label: 'วิเคราะห์หน่วยงาน', desc: 'การกระจุกตัวของผู้รับจ้าง วิธีจัดหา และรูปแบบที่ควรตรวจ', needs: 'agency',
      build: key => {
        const data = aiContextEntity('agency', key);
        return data && {
          data, title: truncate(key, 60),
  ...
  function renderAIStatus() {
    const p = aiProvider();
    const ready = p.kind === 'chrome' || !p.needsKey || !!AI.getKey(ai.cfg.provider);
    const el = U.$('aiStatus');
    el.className = `ai-status ${ready ? 'is-ready' : 'is-missing'}`;
    el.textContent = ready ? `${p.label.split(' (')[0]} · ${p.fixedModel ? 'ในเบราว์เซอร์' : aiModel() || 'ยังไม่เลือกโมเดล'}` : 'ยังไม่ได้ใส่ API key';
  }
  ...
  function wireAISettings() {
    ...
    U.$('aiProvider').addEventListener('change', e => {
      ai.cfg.provider = e.target.value; AI.saveConfig(ai.cfg); renderAISettings();
    });
    U.$('aiKey').addEventListener('change', e => {
      AI.setKey(ai.cfg.provider, e.target.value.trim(), ai.cfg.remember); renderAIStatus();
    });
    ...
    U.$('aiBaseUrl').addEventListener('change', e => {
      ai.cfg.baseUrls[ai.cfg.provider] = e.target.value.trim(); AI.saveConfig(ai.cfg);
    });
    U.$('aiModel').addEventListener('change', e => {
      ai.cfg.models[ai.cfg.provider] = e.target.value.trim(); AI.saveConfig(ai.cfg); renderAIStatus();
    });
  ...
  function aiErrorHTML(err) {
    return `<div class="ai-error"><strong>${U.esc(err.message || 'เกิดข้อผิดพลาด')}</strong>${err.hint ? `<div>${U.esc(err.hint)}</div>` : ''}</div>`;
  }

  /* ---------- งานสำเร็จรูป ---------- */

  function aiCurrentRecord() {
    return ai.lastRecord || profile.record || state.selectedRecord || state.filtered.find(r => r.risk_score > 0) || null;
  }

  function renderAITargets() {
    const r = aiCurrentRecord();
    U.setHTML('aiTargetRecord', r
      ? `<span class="small-muted">สัญญาที่เลือก:</span> ${clickable('project', r.project_id, truncate(r.project_name, 70))} ${scoreBadge(r.risk_score)}`
      : '<span class="small-muted">ยังไม่ได้เลือกสัญญา · เปิดสัญญาใดก็ได้แล้วกด "✨ ถาม AI"</span>');
    const fill = (id, list, keyName, labelName) => {
      const el = U.$(id), keep = el.value;
      U.setHTML(id, list.map(x => `<option value="${U.esc(x[keyName])}">${U.esc(truncate(x[labelName], 60))} · ${U.num(x.n_flagged)}/${U.num(x.n_contracts)} มีสัญญาณ</option>`).join(''));
    ...
    fill('aiTargetContractor', Analytics.contractorTotals(state.filtered).sort(byFlag).slice(0, 60), 'winner_name', 'winner_name');
    fill('aiTargetAgency', Analytics.agencyTotals(state.filtered).sort(byFlag).slice(0, 60), 'dept_name', 'dept_name');
    U.$('aiCartCount').textContent = `${U.num(cart.items.length)} รายการ`;
    U.$('aiTasks').querySelectorAll('[data-ai-task="cart"]').forEach(b => { b.disabled = !cart.items.length; });
    U.$('aiTasks').querySelectorAll('[data-ai-task="contract"], [data-ai-task="letter"]').forEach(b => { b.disabled = !r; });
  }

  function renderAITasks() {
    U.setHTML('aiTasks', AI_TASKS.map(t => `
      <button type="button" class="ai-task" data-ai-task="${t.id}">
  ...
  function runAITask(id, arg) {
    const t = AI_TASKS.find(x => x.id === id);
    if (!t || ai.busy) return;
    let target = arg;
    if (t.needs === 'record') target = arg || aiCurrentRecord();
    if (t.needs === 'contractor') target = arg || U.$('aiTargetContractor').value;
    if (t.needs === 'agency') target = arg || U.$('aiTargetAgency').value;
    if (t.needs === 'cart') { if (!cart.items.length) return; } else if (t.needs && !target) return;
    const built = t.build(target);
    if (!built) return;
    sendAI({ label: t.label + (built.title ? ` · ${built.title}` : ''), prompt: built.prompt, data: built.data, fresh: true });
  }

  /** เปิดแท็บ AI แล้วสั่งงาน — ใช้จากปุ่มในหน้าโปรไฟล์และตะกร้า */
  function openAITask(id, arg) {
  ...
  function aiRefHTML(id) {
    ...
    return exists
      ? `<button type="button" class="ai-ref detail-clickable" data-type="project" data-id="${U.esc(id)}">${U.esc(id)}</button>`
      : `<span class="ai-ref is-unknown" title="ไม่พบรหัสนี้ในข้อมูล AI อาจอ้างผิด">⚠ ${U.esc(id)}</span>`;
  }

  function aiMessageHTML(m, i) {
    if (m.role === 'user') {
      return `<div class="ai-msg is-user" data-i="${i}">
        <div class="ai-bubble">
    ...
    const body = m.error ? aiErrorHTML(m.error) : AI.markdown(m.text, { refFn: aiRefHTML }) || '<span class="ai-typing" aria-label="กำลังคิด"><i></i><i></i><i></i></span>';
    return `<div class="ai-msg is-ai" data-i="${i}">
      <div class="ai-bubble">
        <div class="ai-msg-head"><span class="ai-badge">ร่างโดย AI</span><span class="small-muted">${U.esc(m.meta || '')}</span></div>
        <div class="ai-md">${body}</div>
        ${m.done && !m.error ? `<div class="ai-msg-actions">
          <button type="button" class="btn btn-sm btn-outline-secondary" data-ai-copy="${i}">คัดลอก</button>
          <button type="button" class="btn btn-sm btn-outline-secondary" data-ai-save="${i}">บันทึก .md</button>
  ...
  function paintLastAI() {
    ...
    aiPaintTimer = setTimeout(() => {
      ...
      const el = U.$('aiThread').querySelector(`.ai-msg[data-i="${i}"]`);
      const box = U.$('aiThread');
  ...
  function setAIBusy(busy) {
    ...
    document.querySelectorAll('#aiTasks [data-ai-task], #aiNlBtn').forEach(b => {
      if (busy) b.dataset.wasDisabled = b.disabled ? '1' : '';
      b.disabled = busy || b.dataset.wasDisabled === '1';
    });
    if (!busy) renderAITargets();
  }

  /** ประวัติที่ส่งให้ผู้ให้บริการ — ข้ามคู่คำถามที่ล้มเหลว และตัดให้สั้นลงสำหรับโมเดลในเบราว์เซอร์ */
  ...
  async function sendAI({ label = '', prompt, text, data = '', fresh = false }) {
    ...
    try {
      ...
      if (!reply.text.trim()) reply.error = new AI.AIError('ไม่ได้รับคำตอบจากโมเดล', { hint: 'ลองโมเดลอื่น หรือถามใหม่ให้สั้นลง' });
  ...
  function wireAIChat() {
    const send = () => {
      ...
      input.value = '';
      sendAI({ label: attach ? 'คำถาม · แนบข้อมูลตามตัวกรองปัจจุบัน' : '', text: q, prompt: q, data: attach ? aiContextFilter() : '' });
    };
    U.$('aiSend').addEventListener('click', send);
    U.$('aiInput').addEventListener('keydown', e => {
      if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { e.preventDefault(); send(); }
    });
    U.$('aiStop').addEventListener('click', () => ai.controller?.abort());
    U.$('aiClear').addEventListener('click', () => {
      if (ai.busy) return;
      ai.thread = []; U.$('aiAttach').checked = true; renderAIThread();
    });
    U.$('aiThread').addEventListener('click', async e => {
      ...
      if (!m) return;
      if (copy) {
        try { await navigator.clipboard.writeText(m.text); copy.textContent = 'คัดลอกแล้ว ✓'; }
        catch (err) { copy.textContent = 'คัดลอกไม่ได้'; }
  ...
  /** ปุ่มงาน AI อยู่ทั้งในแท็บ AI หน้าโปรไฟล์ และตะกร้า ต้องผูกตั้งแต่เปิดแอป ไม่ใช่ตอนเปิดแท็บ AI ครั้งแรก */
  function wireAIEntryPoints() {
    document.addEventListener('click', e => {
      ...
      if (btn.closest('#profileDrawer')) {
        ...
        runAITask(btn.dataset.aiTask);
      }
    });
  }

  /* ---------- ค้นหาด้วยภาษาพูด → ตัวกรอง ---------- */
  ...
  function aiFilterVocabulary() {
    ...
    return {
      ...
      method: m.methods || [],
      type: m.project_types || [],
      workGroup: wg ? wg.order.filter(k => wg.counts[k]) : [],
      band: ['priority', ...Rules.BANDS.map(b => b.key)],
      rule: Rules.DEFS.map(d => d.id),
    };
  }

  async function runAIFilter() {
    const q = U.$('aiNlQuery').value.trim();
    if (!q || ai.busy) return;
...
กฎ: ${Rules.DEFS.map(d => `${d.id}=${d.name}`).join(' | ')}`;
    U.setHTML('aiNlResult', '<span class="small-muted">กำลังแปลงคำขอ...</span>');
    setAIBusy(true);
    ai.controller = new AbortController();
    try {
      ...
      renderAIFilterProposal(json);
    } catch (err) {
      U.setHTML('aiNlResult', err.aborted ? '<span class="small-muted">หยุดแล้ว</span>' : aiErrorHTML(err));
    } finally {
      ai.controller = null;
      setAIBusy(false);
    }
  ...
  function validateAIFilter(json) {
    ...
    for (const key of ['province', 'method', 'type', 'workGroup', 'band', 'rule']) {
      ...
      if (v[key].includes(val)) filters[key] = val;
      else rejected.push(`${key}: "${val}"`);
    }
  ...
  function renderAIFilterProposal(json) {
    const { filters, rejected } = validateAIFilter(json);
    const saved = state.filters;
    state.filters = filters;
    const n = state.records.filter(matches).length;
    state.filters = saved;
    const chips = [
      filters.q && `ค้นหา "${filters.q}"`, filters.province, filters.method, filters.type,
      filters.workGroup && `กลุ่มงาน ${workGroupLabel(filters.workGroup)}`,
      filters.band && `ระดับ ${filters.band === 'priority' ? 'ควรตรวจสอบก่อน' : bandLabel(filters.band)}`,
      filters.rule && `กฎ ${filters.rule}`, filters.minValue && `มูลค่า ≥ ${baht(filters.minValue)}`,
      filters.flagged && 'เฉพาะที่พบสัญญาณ',
    ].filter(Boolean);
    ai.pendingFilter = filters;
    U.setHTML('aiNlResult', `
      ${json.explain ? `<p class="mb-1">${U.esc(String(json.explain))}</p>` : ''}
      ...
      ${json.unsupported ? `<div class="ai-warn">แปลงเป็นตัวกรองไม่ได้: ${U.esc(String(json.unsupported))}</div>` : ''}
      <div class="d-flex align-items-center gap-2 mt-2 flex-wrap">
        <button type="button" class="btn btn-sm btn-primary" id="aiNlApply" ${n ? '' : 'disabled'}>ใช้ตัวกรองนี้ · ${U.num(n)} สัญญา</button>
        <span class="small-muted">ตัวกรองเดิมจะถูกแทนที่</span>
      </div>`);
  }

  function applyAIFilter() {
    const f = ai.pendingFilter;
    if (!f) return;
    state.filters = { ...f };
    ...
    applyFilters();
    U.setHTML('aiNlResult', `<span class="ai-ok">ใช้ตัวกรองแล้ว · ${U.num(state.filtered.length)} สัญญา · งานสำเร็จรูปด้านบนจะใช้ขอบเขตใหม่นี้</span>`);
  }

  function wireAIFilter() {
    U.$('aiNlBtn').addEventListener('click', runAIFilter);
    U.$('aiNlQuery').addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); runAIFilter(); } });
    U.$('aiNlResult').addEventListener('click', e => { if (e.target.closest('#aiNlApply')) applyAIFilter(); });
    document.querySelectorAll('[data-ai-example]').forEach(b => b.addEventListener('click', () => {
      U.$('aiNlQuery').value = b.dataset.aiExample; U.$('aiNlQuery').focus();
    }));
  }
  ...
  function renderAI() {
    ...
    U.$('aiScope').textContent = U.$('gfSummary').textContent.replace(/✕|ล้างทั้งหมด/g, '').replace(/\s+/g, ' ').trim();
    renderAITargets();
  }

  ...
  function wireMapAnalysis() {
    ...
    const onAction = e => {
      ...
      if ('mapModeExit' in d) setMapMode('cluster');
        ...
        openAITask('brief');
      } else if ('fpClear' in d) { state.map.footprint = null; drawFootprint(); renderAnalysisPanel(); }
...
- ค่าในบล็อก kpi และ chart ต้องมาจากข้อมูลที่ได้รับเท่านั้น`;
  ...
  const agentTool = name => AGENT_TOOLS.find(t => t.name === name);
...
- รายงานสุดท้ายประกอบด้วย: ## สรุปผลการสืบ, ## หลักฐานที่พบ, ## สิ่งที่ยังไม่ชัดหรือคำอธิบายทางเลือก, ## ขั้นตอนตรวจสอบถัดไป${AI_REPORT_GUIDE}`;
  }
  ...
  function renderAgentReport() {
    ...
    U.setHTML('agentReport', `<div class="ai-msg is-ai"><div class="ai-bubble">
      ...
      <div class="ai-msg-actions">
        ...
        ${r.refs.length ? `<button type="button" class="btn btn-sm btn-outline-secondary" data-agent-cart title="ใส่สัญญาที่รายงานอ้างถึงลงตะกร้า (คุณเป็นผู้กดเอง AI ทำไม่ได้)">🛒 ใส่ตะกร้า ${r.refs.length} สัญญาที่อ้างถึง</button>` : ''}
  ...
  async function runAgent() {
    ...
    const tools = AGENT_TOOLS.map(({ name, description, schema }) => ({ name, description, schema }));
    const t0 = performance.now();
    ...
    try {
      for (let round = 0; round < maxSteps + 3; round++) {
        ...
        for (const call of res.calls) {
          ...
          try {
            ...
            text = `[S${stepNo}] ${json}`;
            entry.summary = tool.summarize(out, call.input || {});
      ...
      upsertAIHistory({ id: agent.id, kind: 'agent', at: new Date().toISOString(), title: goal, scope: scopeText, meta: agent.report.meta, answer: finalText, verify: agent.report.verify,
        steps: agent.entries.filter(e => e.type === 'tool').map(e => ({ n: e.n, name: e.name, label: agentTool(e.name)?.label || e.name, summary: e.summary, error: !!e.error, ms: Math.round(e.ms) })) });
    } catch (err) {
  ...
  function wireAgent() {
    ...
    U.setHTML('agentToolList', AGENT_TOOLS.map(t => `<li><span aria-hidden="true">${t.icon}</span> <b>${U.esc(t.label)}</b> <code>${t.name}</code><br><span class="small-muted">${U.esc(t.description)}</span></li>`).join(''));
    ...
    U.$('agentReport').addEventListener('click', async e => {
      ...
      if (e.target.closest('[data-agent-cart]')) {
        ...
        const steps = agent.entries.filter(x => x.type === 'tool').map(x => `- S${x.n} ${agentTool(x.name)?.label || x.name} (${agentArgsText(x.input)}) → ${x.summary || ''}`).join('\n');

/* rules.js — เครื่องมือประเมินความเสี่ยง R1-R18
...
const Rules = (() => {
  ...
  const DEFS = [
    ...
    },
    {
      ...
      thresholds: {
        ...
        minShare: { value: 0.30, min: 0.05, max: 0.90, step: 0.05, label: 'สัดส่วนขั้นต่ำของพื้นที่', format: 'pct' },
      },
      logic: t => `สัญญานี้ ราคา = ราคากลางพอดี และในพื้นที่ (ประเภทงาน x จังหวัด) ` +
        `สัดส่วนแบบนี้ >= ${t.minShare} และสูงกว่าค่ากลางประเทศ >= ${t.minGapPts} จุด % ` +
        `(พื้นที่ต้องมีอย่างน้อย 20 สัญญา)`,
      evaluate(r, ctx, t) {
        ...
        if (!stat) return null;                        // ข้อมูลในพื้นที่นี้น้อยเกินกว่าจะเทียบได้
        if (stat.share < t.minShare) return null;
        if (stat.gapPts < t.minGapPts) return null;
        return {
          actual: `${r.province}: ${U.pct(stat.share)} ของสัญญา (${stat.exact} จาก ${stat.n}) ปิดราคาเท่าราคากลาง ` +
            `ขณะที่ทั้งประเทศ ${U.pct(stat.national)} — สูงกว่า ${stat.gapPts.toFixed(0)} จุด %`,
        };
      }
    },
  ];
  ...
  function buildContext(records) {
    ...
    return Object.assign(
      { projectContracts, tinToNames, nameToTins, agencyMethod, splitGroups, pairCounts, provinceCentroid },
  ...
  function buildRegionalPrice(records) {
    ...
    for (const [cell, s] of cellStat) {
      ...
      regionalExact.set(cell, {
        ...
        gapPts: (share - nat) * 100,     // สูงกว่าค่ากลางประเทศกี่จุดเปอร์เซ็นต์
      });
    }
    return { regionalExact, nationalExact, REGIONAL_MIN_GROUP };
  }

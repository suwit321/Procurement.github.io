...
const DataIO = (() => {
  ...
  /** เลขไทย (๐-๙) → เลขอารบิก — เอกสารราชการไทยบางแห่งยังพิมพ์ตัวเลขเป็นเลขไทย
   *  โดยเฉพาะไฟล์ที่แปลงมาจาก PDF หรือพิมพ์ในโหมดฟอนต์ไทย เจอจริงในชื่อโครงการของชุดข้อมูลนี้เอง
   *  (เช่น "หมู่ที่ ๑", "๑๐ ลูกบาศก์เมตร") ถ้าไม่แปลงก่อน Number()/regex \d จะไม่รู้จักเลขไทยเลย
   *  กลายเป็นค่าว่างทั้งที่ข้อมูลมีอยู่จริง — ใช้เฉพาะจุดที่ตีความค่าเป็นตัวเลข/วันที่ ไม่แตะข้อความแสดงผล
   *  (ชื่อโครงการ/หน่วยงาน/ผู้รับจ้าง) เพื่อคงข้อความต้นฉบับไว้ตามที่ผู้ใช้ป้อนมา */
  ...
  function parseMoney(value) {
    ...
    const text = thaiDigitsToArabic(value).trim().replace(/,/g, '').replace(/^฿/, '').replace(/บาท$/, '').trim();
    const n = Number(text);
  ...
  function parseThaiDate(value, stats) {
    ...
    const text = thaiDigitsToArabic(value).trim();

    // แบบที่ 1 — ไทยย่อ/ไทยเต็ม '20 พ.ย. 68' (เส้นทางเดียวกับ ETL) — รับ '๒๐ พ.ย. ๖๘' ด้วยเพราะแปลงเลขไทยไว้แล้วข้างบน
